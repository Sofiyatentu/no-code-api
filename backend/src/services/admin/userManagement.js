...
export const getUserProfile = async (userId) => {
  try {
    ...
    const statusResult = await query(
      "SELECT status FROM user_statuses WHERE user_id = $1",
      [userId],
    );
    const status = statusResult.rows[0]?.status || "active";

    const rateLimitsResult = await query(
      "SELECT * FROM rate_limit_configs WHERE user_id = $1",
      [userId],
    );
    ...
    const flowsResult = await query(
      "SELECT COUNT(*) FROM flows WHERE user_id = $1",
      [userId],
    );
  ...
  try {
    ...
    const projectsWithStats = await Promise.all(
      projects.map(async (project) => {
        const flowsResult = await query(
          "SELECT id, name, deployed FROM flows WHERE project_id = $1",
          [project.id],
        );
        const requestsResult = await query(
        ...
        const lastRequestResult = await query(
          "SELECT timestamp FROM logs WHERE project_id = $1 ORDER BY timestamp DESC LIMIT 1",
          [project.id],
        );

        return {
          ...project,
          flowCount: flowsResult.rows.length,
          deployedFlows: flowsResult.rows.filter((f) => f.deployed).length,
          totalRequests: Number(requestsResult.rows[0].count),
          lastRequest: lastRequestResult.rows[0]?.timestamp || null,
        };
      }),
    );
...
export const getUserFlows = async (userId) => {
  try {
    const flowsResult = await query(
      "SELECT id, name, deployed, deployed_at, version FROM flows WHERE user_id = $1",
      [userId],
    );
    ...
    const flowsWithStats = await Promise.all(
      flows.map(async (flow) => {
        const requestsResult = await query(
          "SELECT status, duration FROM logs WHERE flow_id = $1",
          [flow.id],
        );
        const requests = requestsResult.rows;
        const lastRunResult = await query(
          "SELECT timestamp FROM logs WHERE flow_id = $1 ORDER BY timestamp DESC LIMIT 1",
          [flow.id],
        );
        ...
        return {
          ...
          avgResponseTime:
            requests.length > 0
              ...
              : 0,
          lastRun: lastRunResult.rows[0]?.timestamp || null,
        };
      }),
    );
...
export const calculateUserUsage = async (userId, startDate, endDate) => {
  try {
    const logsResult = await query(
      "SELECT status, duration, request_body FROM logs WHERE user_id = $1 AND timestamp >= $2 AND timestamp <= $3",
    ...
    const totalDataProcessed = logs.reduce(
      (sum, l) => sum + JSON.stringify(l.request_body || "").length,
      0,
    );
    ...
    return {
      ...
      successRate:
        logs.length > 0
          ? ((successfulRequests / logs.length) * 100).toFixed(2)
          : 0,
      avgResponseTime,
  ...
  try {
    ...
    if (checkResult.rows.length > 0) {
      result = await query(
        ...
        [
          quotaData.requests_per_minute,
          quotaData.requests_per_hour,
          quotaData.requests_per_day,
          quotaData.tier,
          userId,
        ],
      );
      ...
      result = await query(
        ...
        [
          userId,
          quotaData.requests_per_minute || 60,
          quotaData.requests_per_hour || 1000,
          quotaData.requests_per_day || 10000,
          quotaData.tier || "free",
        ],
      );
    }
