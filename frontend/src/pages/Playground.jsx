...
//       const config = {
//         method,
//         url: `http://localhost:5000/api/${user.username}/${projectSlug}${path}`,
  ...
  const handleSend = async () => {
    ...
    try {
      if (!user?.username || !project?.slug) {
        throw new Error("Missing username or project slug");
      }

      const url = `http://localhost:5000/${user.username}/${project.slug}${path}`;
              ...
              {(method === "POST" ||
                ...
                <div>
                  ...
                  <textarea
                    value={body}
                    onChange={(e) => setBody(e.target.value)}
                    className="w-full px-4 py-2 rounded bg-slate-800 border border-slate-700 font-mono text-sm"
                    rows={6}
                    placeholder='{"key": "value"}'
                  />
                </div>
