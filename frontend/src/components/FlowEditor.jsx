...
const NODE_TYPES = {
  ...
  mongodb: { label: "MongoDB", color: "bg-green-900" },
...
const CustomNode = ({ data, isConnecting, id }) => {
  const { nodeType, onNodeClick, isExecuting, isCompleted, executionOrder } =
    data;

  return (
    <div
      className={`px-4 py-2 rounded border-2 transition-all duration-300 relative min-w-[120px] cursor-pointer ${
        isExecuting
      ...
      {nodeType === "tryCatch" && (
        <>
          ...
          <Handle
            ...
            style={{ top: "70%" }}
          />
          <div className="absolute right-[-60px] top-[20%] text-xs text-cyan-300">
            Try
          </div>
          <div className="absolute right-[-60px] bottom-[20%] text-xs text-cyan-300">
            Catch
          </div>
        </>
      )}
      {nodeType === "condition" && (
        <>
          ...
          <Handle
            ...
            style={{ top: "70%" }}
          />
          <div className="absolute right-[-40px] top-[20%] text-xs text-cyan-300">
            If
          </div>
          <div className="absolute right-[-50px] bottom-[20%] text-xs text-cyan-300">
            Else
          </div>
        </>
      )}
...
export default function FlowEditor({ flow }) {
  ...
  // Initialize nodeConfigs from saved node data when nodes change
  useEffect(() => {
    const configs = {};
    nodes.forEach((node) => {
      if (node.data?.config && !nodeConfigs[node.id]) {
        configs[node.id] = node.data.config;
      }
    });
    if (Object.keys(configs).length > 0) {
      setNodeConfigs((prev) => ({ ...prev, ...configs }));
    }
  }, [nodes]);
  ...
  const hasParent = (edges, nodeId) => {
    return edges.some((edge) => edge.target === nodeId);
  };

  // detect cycles
  const createsCycle = (edges, source, target) => {
  ...
  const onConnect = useCallback(
    (connection) => {
      ...
      // RULE 1: each node must have only one parent
      if (hasParent(edges, target)) {
        alert("This node already has a parent.");
        return;
      }

      // RULE 2: prevent cycles
      if (createsCycle(edges, source, target)) {
        alert("Cycle detected. Flow must be acyclic.");
        return;
      }
      const sourceNode = nodes.find((n) => n.id === source);

      if (sourceNode?.data?.nodeType === "response") {
        alert("Response node cannot have children.");
        return;
      }
  ...
  const onNodesDelete = useCallback(
    (deletedNodes) => {
      ...
      setEdges(newEdges);
      dispatch(
        pushHistory({ flowId: flow.id, nodes: newNodes, edges: newEdges }),
      );
    },
  ...
  const handleSaveFlow = useCallback(() => {
    ...
    // Merge nodeConfigs back into node data before saving
    const mergedNodes = nodes.map((node) => {
      if (nodeConfigs[node.id]) {
        return {
          ...node,
          data: { ...node.data, config: nodeConfigs[node.id] },
        };
      }
      return node;
    });
    console.log(
      "API Logic JSON:",
      JSON.stringify({ nodes: mergedNodes, edges }, null, 2),
    );
    dispatch(
      updateFlow({ flowId: flow.id, data: { nodes: mergedNodes, edges } }),
    )
      .then(() => {
  ...
  }, [dispatch, flow.id, nodes, edges, nodeConfigs, setNodes]);
  ...
  const handleNodeClick = useCallback(
    (nodeId) => {
      // Load saved config from node data when opening config panel
      const node = nodes.find((n) => n.id === nodeId);
      if (node?.data?.config && !nodeConfigs[nodeId]) {
        setNodeConfigs((prev) => ({
          ...prev,
  ...
  const addChildNode = useCallback(
    (parentNodeId, branchType) => {
      ...
      if (hasParent(edges, childNode.id)) {
  ...
  const simulateFlowExecution = useCallback(async () => {
    ...
    const traverseFlow = async (currentNodeId, visited = new Set()) => {
      ...
      const processingTime =
        currentNode.data.nodeType === "mongodb"
          ? 1500
          : currentNode.data.nodeType === "response"
            ? 800
            : currentNode.data.nodeType === "transform"
              ? 1000
              : 500;
      ...
      if (currentNode.data.nodeType === "tryCatch") {
        ...
        const tryEdge = outgoingEdges.find(
          (edge) => edge.sourceHandle === "try",
        );
        if (tryEdge) {
  ...
  const createCompleteAPIScenario = useCallback(() => {
    ...
    newEdges.push(
      // Main flow: HTTP -> Try/Catch
      {
        id: `edge-1-${Date.now()}`,
        source: httpNode.id,
        target: tryCatchNode.id,
      },

      // TRY branch: Try/Catch -> MongoDB -> Condition
      {
        id: `edge-2-${Date.now()}`,
        source: tryCatchNode.id,
        target: mongoNode.id,
        sourceHandle: "try",
      },
      {
      ...
      // Condition branches: Condition -> Success Response (IF) and Not Found Response (ELSE)
      {
        ...
        target: successResponseNode.id,
        sourceHandle: "if",
      },
      {
        id: `edge-5-${Date.now()}`,
        source: conditionNode.id,
        target: notFoundResponseNode.id,
        sourceHandle: "else",
      },
    ...
    <div className="w-full h-full bg-slate-800 rounded-lg overflow-hidden flex flex-col">
      ...
      <div className="border-b border-slate-700 p-4 flex gap-2 items-center bg-slate-900">
        ...
        <button
          ...
          className={`ml-auto px-4 py-1.5 rounded text-white text-sm font-semibold flex items-center gap-2 transition ${
            saveStatus === "saved"
              ? "bg-emerald-600"
              : saveStatus === "saving"
                ? "bg-cyan-600"
        ...
        <ReactFlow
          nodes={nodes.map((n) => ({
            ...
            data: {
              ...
              isExecuting:
                executionState?.currentNodeId === n.id &&
                executionState?.isRunning,
              isCompleted:
                executionState?.path.includes(n.id) &&
                !executionState?.isRunning,
          ...
          edges={edges.map((e) => ({
            ...
            style: {
              stroke:
                executionState?.path.includes(e.source) &&
                executionState?.path.includes(e.target)
      ...
      {configPanel && (
        <div
          className={`absolute bottom-4 right-4 bg-slate-900 border border-slate-700 rounded-lg transition-all duration-300 ease-in-out ${
            sidebarExpanded
              ? "w-96 p-4 max-h-96 overflow-y-auto"
              : "w-12 p-2 max-h-12 overflow-hidden"
          }`}
        >
          {sidebarExpanded ? (
            <>
              ...
              {(() => {
                ...
                switch (node.data.nodeType) {
                  ...
                  case "mongodb":
                    return (
                      <div className="space-y-4">
                        <div className="bg-green-900/30 px-3 py-2 rounded border border-green-500/50">
                        ...
                        {(config.operation === "find" || !config.operation) && (
                          <>
                            <div>
                              <label className="block text-sm font-medium text-slate-300 mb-1">
                                Query (JSON)
                              ...
                              <textarea
                                ...
                                placeholder="{} or { email: request.body.email }"
                        ...
                        {config.operation === "insert" && (
                          <div>
                            ...
                            <textarea
                              ...
                              placeholder="{ name: request.body.name, email: request.body.email }"
                          ...
                          <>
                            <div>
                              ...
                              <textarea
                                ...
                                placeholder="{ _id: request.params.id }"
                            ...
                            <div>
                              ...
                              <textarea
                                ...
                                placeholder="{ $set: { name: request.body.name } }"
                                className="w-full px-3 py-2 bg-slate-800 border border-slate-600 rounded text-white text-sm h-20 font-mono"
                              />
                            </div>
                          </>
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 mt-1">
                            Variable name to store results (use in response as{" "}
                            {`{{varName}}`})
                          </p>
                        </div>
                      </div>
                  ...
                  case "condition":
                    const ifChild = nodes.find(
                      (n) =>
                        n.data.parentNodeId === configPanel &&
                        n.data.branchType === "if",
                    );
                    const elseChild = nodes.find(
                      (n) =>
                        n.data.parentNodeId === configPanel &&
                        n.data.branchType === "else",
                    );
                  ...
                  case "tryCatch":
                    const tryChild = nodes.find(
                      (n) =>
                        n.data.parentNodeId === configPanel &&
                        n.data.branchType === "try",
                    );
                    const catchChild = nodes.find(
                      (n) =>
                        n.data.parentNodeId === configPanel &&
                        n.data.branchType === "catch",
                    );
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              {catchChild ? (
                                ...
                                <button
                                  onClick={() =>
                                    addChildNode(configPanel, "catch")
                                  }
                                  className="px-2 py-1 text-xs bg-slate-600 hover:bg-slate-700 rounded"
                                >
                                  Add Node
                    ...
                    return (
                      <div className="space-y-4">
                        ...
                        <div>
                          ...
                          />
                          <p className="text-xs text-slate-500 mt-1">
                            Use {`{{varName}}`} to insert variables. Example:{" "}
                            {`{ "data": "{{users}}" }`}
                          </p>
                        </div>
                        <div>
                    ...
                    return (
                      <div className="space-y-4">
                        <div>
                          ...
                          />
                          <p className="text-xs text-slate-500 mt-1">
                            JavaScript function body for data transformation
                          </p>
                        </div>
                      </div>
                    );
                      ...
                      <div className="space-y-4">
                        <div className="bg-slate-800 p-3 rounded border border-slate-600">
                          ...
                          <p className="text-xs text-slate-300">
                            This is a {node.data.branchType} block. Add nodes
                            after this block to define the execution flow.
                          </p>
                        </div>
                      </div>
                    );
